...
void WyszukajWszerz::wykonajAlgorytm(Graf graf)
{
	fstream przebieg;
	...
	queue<int> kolejka, kolejkaKopia;
	...
	while (!kolejka.empty())
	{
		...
		kolejkaKopia = kolejka;

		przebieg << "Stan kolejki: ";
	
		if (kolejkaKopia.empty())
		{
			przebieg << "Kolejka jest pusta\n";
		}
		else
		{
			while (!kolejkaKopia.empty())
			{
				przebieg << kolejkaKopia.front() << " ";
				kolejkaKopia.pop();
			}
			przebieg << "\n";
		}
